...
이 측정은 전체 아이템 삭제가 아니라 **점유 슬롯 벡터를 정리하는 과정**만 비교한 결과입니다. 측정 당시 별도로 남아 있던 `RemoveItem(Item*)`의 포인터 선형 탐색은 이후 `ItemHandle`을 도입하여 제거했으며, 그 결과는 [핸들 조회 벤치마크](LOOKUP_RESULTS.md)에 분리해 기록했습니다.
