...
`InventoryBenchmark`의 `LinearScanInventory` 역시 강의 원본을 복원하거나 복사한 코드가 아닙니다. 선형 탐색 방식과 현재 구현의 성능을 비교하기 위해 직접 작성한 코드입니다.
...
핸들은 슬롯 번호와 세대 번호로 구성됩니다. 삭제된 슬롯이 다시 사용되면 세대 번호가 바뀌므로, 오래된 핸들이 새 아이템을 잘못 조회하거나 삭제하는 것을 막을 수 있습니다. 대신 슬롯별 세대 번호를 저장할 추가 메모리와 상태 관리가 필요하므로, 메모리 사용과 조회 성능 사이의 트레이드오프가 있습니다.
...
소유권 이동, 삭제 시 정확히 한 번 소멸, 가득 찬 인벤토리가 거절한 아이템의 자동 해제도 검증합니다.
...
선형 탐색 방식과 빈 슬롯 추적 방식을 동일한 조건에서 비교합니다. 콘솔 출력과 아이템 생성 비용은 측정에서 제외하며, 슬롯 수와 점유율별 아이템 추가 시간을 연산 1회당 나노초인 `ns/op`로 출력합니다.
