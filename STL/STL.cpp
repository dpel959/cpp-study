...
int main()
{
	...
	voidFuncType* ptr = &Print; // Print로도 가능. 함수 이름이 함수 포인터로 변환되는 표준 C++ 문법임.
	...
	//void(*FuncPtrType)(); // 이렇게 함수 포인터 변수 자체 선언이 가능하다. 다만 문법이 복잡해서 using을 쓰는 편이 읽기 좋음.
...
///
/// 콜백 함수. 어떤 행동을 했을때 역으로 함수를 호출해주는 함수.
/// 
...
int main()
{
	...
	// Q. 어? 이거 될 거 같은데 왜 안 돼죠? 타입이 맞지 않다고 하는데 타입 맞고, 테스트 클래스의 것이란 것도 명시해줬잖아요!
	//
	// A. 멤버 함수는 호출할 대상 객체가 필요하므로 정적/전역 함수와 타입이 다르다.
	// MSVC 32비트에서는 일반 함수와 멤버 함수의 호출 규약 차이도 있지만, 64비트까지 무조건 cdecl/thiscall로 나뉘는 것은 아니다.
	// 
	// PrintTest는 Test 객체를 대상으로 실행하는 Test 객체의 종속적인 함수다.
	// 그래서 실제로는, 함수를 호출할 때 'Test 객체의 주소까지 같이 넘긴다.'
	// 첫 번째 매개변수로 자기 자신의 주소를 넘긴다.
	// 
	// 그러므로 '아니, 함수 타입 맨 앞에 나 자신이 없잖아!! < 이렇게 되는 것이다.
	// 
	...
	// => 이건 '지연 실행'의 예시이다. 다른 스레드나 작업 시스템에서 처리한다면 비동기 처리에도 사용할 수 있다.
...
/// 
/// predicate는 조건을 판별하고, 결과가 bool처럼 사용될 수 있는 호출 가능한 것을 말한다.
/// 조건에 맞는 시그니처라면 람다, 함수 포인터, 함수 객체 아무거나 넣어도 상관 없다.
/// 
/// Functor은 상태를 저장하거나 operator를 여러 형태로 만들 수 있어서 함수 포인터보다 유연한 경우가 많다.
/// 그렇다고 모든 상황에서 무조건 함수 포인터의 상위 호환이라는 뜻은 아니다.
/// 
...
{
public:
	virtual ~Job() {} // virtual은 무조건. 그냥 상속이 될 상위 객체면. 소멸자에 무조건 쓰자.
...
#if 0
	// 이건 문제가 여러가지 있다. 1. it는 지워졌는데, it가 가리키는 곳은 그대로이고, erase시 벡터는 당겨져있다.
...
int main()
{
	...
	std::pair<int, Player*> entry(v[0]->_id, v[0]); // 별 거 없음. 그냥 2가지를 담는 컨테이너임. first, second를 접근 가능.
	// 당연히 값은 둘 다 넣어줘야 함.
	m.insert(entry);
	...
	// 하지만! Map은 RBT 기반. 애초에 순회를 해줄 필요가 없다.

	// 찾기
	// 찾기의 시간 복잡도는 당연히 O(logN). 높이가 무조건 logN이 보장됨. RBT니까. 한번 내려갈때마다 후보가 반이 없어진다.
	// 벡터 O(N)보다 좋다!
	...
	// C++ STL은 그런데, UE TMap이란 게 있는데, 얘는 없는 key를 넣으면 크래시가 난다!
	// 다른 것도 있는데, STL에서 empty()는 비었는지 안 비었는지 확인하는 건데, UE에서는 clear()이다. 나중에 구별해주도록 하자.
	...
	// 다음 노드를 찾는 과정이 필요하지만, map 전체 순회의 시간 복잡도도 O(N)이다.
	// 다만 벡터와 다르게 메모리에 일렬로 붙어 있지 않아서, 실제 순회 성능은 벡터가 유리한 경우가 많다.
...
int main()
{
	...
	// 맞는데, 외부에서 key를 통해 접근할때의 이야기이다. begin()은 첫 원소를 가리키는 iterator를 줌.
	// 그 뒤 iterator를 증가시키며 전체 원소를 O(N)에 순회할 수 있다.
	// 노드와 버킷이 실제로 어떻게 연결되는지는 STL 구현체마다 다르므로, 특정 내부 구조를 전제로 하면 안 된다.
...
int main()
{
	...
	// 런타임 중 RTTI를 사용하므로 비용은 있지만, 실제로 문제가 될 정도인지는 상황에 따라 측정해야 한다.
