...
화면에 탱크와 포탄을 그리는 예제에서 끝내지 않고, 객체의 소유권과 갱신 시점에서 발생할 수 있는 문제를 다시 검토했습니다. 강의의 바람과 중력이 적용된 포탄 이동을 바탕으로 굴곡진 지형, 충돌과 체력, 승패와 재시작까지 더해 한 판의 게임 흐름이 완성되도록 확장했습니다.
...
- 파일 열기·읽기 실패, 음수인 선 개수와 좌표 형식 오류에 대한 `LineMesh` 입력 검증 추가
